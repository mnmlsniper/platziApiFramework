...
test.describe('Products', () => {
  test('get all products returns array', async ({ api }) => {
    const { response, body } = await api.products.getAll({ limit: 10 });
  ...
  test('get product by id', async ({ api }) => {
    ...
    const { response, body } = await api.products.getById(list[0].id);
  ...
  test('CRUD: create → update → delete', async ({ api }) => {
    const { response: createRes, body: product } = await api.products.create({
    ...
    const { response: updateRes, body: updated } = await api.products.update(product.id, {
      title: 'Updated Title',
      price: 150,
    ...
    const { response: deleteRes } = await api.products.delete(product.id);
    expect(deleteRes.status()).toBe(200);
  });

  test('filter products by price range', async ({ api }) => {
    const { response, body } = await api.products.filterByPriceRange(10, 100);
